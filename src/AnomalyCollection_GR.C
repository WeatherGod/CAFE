...
#include <StrUtly.h>				// for TakeDelimitedList(), ToUpper(), StrToDouble(), Size_tToStr(), RandomStr()
...
#include <FormatUtly.h>				// for Underline() and Bold()
...
// This funcrion will save the data to the mysql database.
{
	...
	{
		cerr << "ERROR: Problem while reading the peakvalley file: " << ProgOutputName << endl;
		return(false);
	}
	
	// TODO: Refactor this into the TrainingInterface class
...
//		cout << '+' << LevelName << '+' << endl;
		...
		// replacing the commas with spaces.
                LevelArgs += GiveDelimitedList( TakeDelimitedList( ConfigInfo.Give_DataSource_DataLevel(CAFEVarName, LevelName),
                                                                   ','), 
                                                ' ') 
                           + ' ';
        }

	return(DataVarName + ' ' + Size_tToStr(TakeDelimitedList(RipWhiteSpace(LevelArgs), ' ').size()) + ' ' + LevelArgs);
}
...
{
	const string SysCommand = "gradsnc -lbc '" + ProgName + ' ' + DateTimeStr + ' ' 
					+ Size_tToStr(VarCount) + ' ' + CmdLineArg + "' >& " + ProgOutputName;
...
{
	// Working on eliminating the need for this statement.
	...
	for (size_t ArgIndex = 0; ArgIndex < CommandArgs.size(); ArgIndex++)
	{
		...
		else if (CommandArgs[ArgIndex].find("--round=") == 0)
		{
			...
			{
				cerr << "ERROR: Invalid number for rounding: " << RoundingHourValue << endl;
				cerr << "       Must be a value greater than 0." << endl;
				return(8);
			}
		}
		else if (CommandArgs[ArgIndex].find("--help") == 0)
		{
	...
	if (!ConfigInfo.SetDefaultSource("NCEP_Global"))
	{
		cerr << "ERROR: Couldn't set NCEP_Global as a default data source\n";
		return(1);
	}
	...
	const string PeakValProgName = CAFEOptions.CAFEPath + "/GradScripts/MasterControl.gs";
	...
	try
	{
		...
		for (vector<string>::const_iterator AnEventType = UniqueTypesList.begin(); AnEventType != UniqueTypesList.end(); AnEventType++)
		{
